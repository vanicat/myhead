...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduleshapes.code.tex,v 1.19 2011/07/21 11:44:47 tantau Exp $
...
\def\pgfmultipartnode#1#2#3#4{%
  ...
  {%
    {%
      ...
      \fi%
      \let\pgfpositionnodelaterpostsetup\pgfutil@empty% TT
...
%   
%   \pgfpositionnodelaterpostsetup
%   Actually, this macro will not be setup especially for the call of
%   #1, except that is set to \pgfutil@empty at the beginning of every
%   node. Rather, you can store code in this macro that should be
%   executed after the final positioning has been done. What happens
%   is that this macro is actually just executed when
%   \pgfpositionnodenow is called later on and, just as for the other
%   of the above macros, it is the job of #1 to ensure that all of
%   these macros (including this one) have the same value when
%   \pgfpositionnodenow is called that they had when #1 was called.
%
...
\let\pgfpositionnodelaterpostsetup\pgfutil@empty
...
\long\def\pgf@sh@beforefgpath#1{\expandafter\gdef\csname pgf@sg@ffg@\pgf@sm@shape@name\endcsname{#1}}
